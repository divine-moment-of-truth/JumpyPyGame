...
class Game:
    ...
    def new(self):
        ...
        for plat in PLATFORM_LIST:
            Platform(self, *plat) # * - this means explode the list
        self.mob_timer = 0
    ...
    def update(self):
        ...
        if now - self.mob_timer > 5000 + random.choice([-1000, -500, 0, 1000]):
            self.mob_timer = now
            Mob(self)
        # hit mobs?
        ...
        if self.player.vel.y > 0:
            hits = pg.sprite.spritecollide(self.player, self.platforms, False) # False means that the sprite will not be deleted
            if hits:
                lowest = hits[0]
                for hit in hits:
        ...
        if self.player.rect.top <= HEIGHT / 4:
            ...
            for mob in self.mobs:
                # moves teh clouds half the speed of the player
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN:
                if event.key == pg.K_SPACE:
                    self.player.jump()
                    # If you press the space bar and release quickly then you will not jump as high
